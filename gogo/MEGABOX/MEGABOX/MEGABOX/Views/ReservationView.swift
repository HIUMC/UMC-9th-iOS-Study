...
// MARK: - New ReservationView (JSON-backed, using ReservationViewModel)
struct ReservationView: View {
    @Environment(NavigationRouter.self) var router
...
private extension ReservationView {
    var MovieSelectionSection: some View {
        VStack(spacing: 0) {
            // 상단의 영화 정보 영역
            HStack(spacing: 0) {
                // 연령제한 박스 (선택 영화의 관람등급 표시)
                ZStack {
                    RoundedRectangle(cornerRadius: 4)
                        .frame(width: 26, height: 24)
                        .foregroundStyle(.orange)
                    Text((viewModel.pickedMovie?.rating ?? "").isEmpty ? "-" : (viewModel.pickedMovie?.rating ?? "-"))
                ...
                // 영화 이름 표시 (선택된 영화가 없다면 기본 텍스트)
                Text(viewModel.pickedMovie?.title ?? "영화 선택")
                    .font(.PretendardBold(size:18))
                    .foregroundStyle(.black)
...
// MARK: - 날짜 선택 (ReservationViewModel 연결 버전)
private struct ReservationWeekSelectorView: View {
            ...
            HStack(spacing: 8) {
                ForEach(viewModel.weekDays, id: \.self) { day in
                    ...
                    ReservationDateButton(
                        date: day,
                        base: viewModel.weekDays.first ?? day,
...
// MARK: - 날짜 버튼 (기준일 = JSON 시작일)
private struct ReservationDateButton: View {
    let date: Date
    let base: Date            // viewModel.weekDays.first
    let isSelected: Bool
    let onTap: () -> Void

    var body: some View {
        let cal = Calendar.current
        let isToday = cal.isDate(date, inSameDayAs: base)
        let isTomorrow = cal.isDate(date, inSameDayAs: cal.date(byAdding: .day, value: 1, to: base) ?? base)
        let weekday = cal.component(.weekday, from: date)

        let main = isToday ? monthDayString(date) : dayString(date)
        let sub = isToday ? "오늘" : (isTomorrow ? "내일" : weekdayKOR(date))

        let textColor: Color = {
            if isSelected { return .white }
            if weekday == 1 { return .red }
            if weekday == 7 { return .tag }
            return .primary
        }()
    ...
    private func weekdayKOR(_ d: Date) -> String {
        let f = DateFormatter()
        f.locale = Locale(identifier: "ko_KR")
        f.dateFormat = "E"
        return f.string(from: d)
