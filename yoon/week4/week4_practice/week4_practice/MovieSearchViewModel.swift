...
final class MovieSearchViewModel: ObservableObject {
    private let model: [MovieModel] = [
        ...
        .init(movieImage: .init(.brutalist), title: "브루탈리스트", rate: 8.2),
    ...
    init() {
        $query
            ...
            .removeDuplicates()
            .handleEvents(receiveOutput: { [weak self] _ in self?.errorMessage = nil
            })
            .flatMap { query in
    ...
    private func search(query: String) -> AnyPublisher<[MovieModel], Error> {
        ...
        .handleEvents(
            receiveSubscription: {_ in DispatchQueue.main.async { self.isloading = true }}
        ).eraseToAnyPublisher()
    }
}
